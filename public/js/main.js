...
------------------------------ */
	$("#contactForm").on("submit", function (event) {
		if (event.isDefaultPrevented()) {
			...
			submitForm();
            $('#contactForm input,#contactForm textarea').not('#paramContact').val('');
		}
	});
	function submitForm(){
		var name = $("#name").val();
		...
		var id = $("#paramContact").val();


		$.ajax({
			type: "POST",
			url: "http://olymposweb.com:81/contact_us/contact",
			data: "name=" + name + "&email=" + email + "&msg_subject=" + msg_subject + "&message=" + message + "&id=" + id,
